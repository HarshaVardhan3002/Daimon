...
| Send/retry | The current `/chat` test contract returns a full completion rather than token streaming. On Send, persist the request snapshot, immediately clear the submitted composer text/attachment, and show the authored prompt as a pending user bubble. Success fills that same turn without duplication; failure and Stop keep the prompt visible and retryable. Later text typed into the composer is independent and survives the request outcome. Stop aborts the pending HTTP request. |
...
| **1 — Ordinary conversation** | General prompt, immediate authored user bubble, pending state with Stop, full assistant response, copy, error/retry, New chat, search and Recents. | With the optional local proxy available, an arbitrary prompt and recent answered context produce a real response in the same authored turn. If disconnected or upstream fails, keep the failed prompt visible with a plain-language retry action; Stop keeps it visible as stopped. Text typed after Send survives success and failure. Retry reuses the same turn without duplication, and unanswered turns do not enter later model context. No canned fixture answer appears in the main Send path; no streaming is promised. No visible dead control. |
...
- Keep one persisted draft per chat. New chat and chat switching preserve unsent text. Sending captures the current prompt and attachment, then clears those submitted composer values immediately; new text typed while the request runs remains in the composer through success, failure, cancellation, and retry.
- Persist the request snapshot separately from the visible turn list and create a persisted authored user bubble immediately. Success fills that same turn with the assistant answer, never appending a duplicate. Failure keeps the prompt visible as failed; Stop keeps it visible as stopped. Both retain the request identity for retry where the error is recoverable. On restart, an interrupted pending turn is shown as stopped and can retry from its stored request snapshot.
- Keep prompt and answer semantics clear: the user bubble is the authored prompt from Send; the assistant answer appears below it only after a valid completion. A pending, failed, or stopped user turn is not presented as a successful assistant response.
