...
export type PendingChatRequest = { messages: ChatMessage[]; prompt: string; locale: 'en' | 'de'; draftSnapshot: string; attachment?: ImageAttachment; documentAttachment?: DocumentAttachment; documentContextAttachment?: DocumentAttachment; replacementTurnId?: string; status: 'loading' | 'error'; error?: ChatRequestError };
export type ChatSession = { sampleSourceSelected?: boolean; activeDocumentContext?: DocumentAttachment; pendingLiveRequest?: PendingLiveRequest; pendingChatRequest?: PendingChatRequest };
...
export function decodeChatSession(value: unknown): ChatSession {
  ...
  if (value.pendingChatRequest !== undefined) {
    const pendingChat = value.pendingChatRequest;
    ...
    const pendingChatRequest: PendingChatRequest = {
    ...
    return { ...decodeLegacyLiveRequest(value, sampleSourceSelected), ...(activeDocumentContext ? { activeDocumentContext } : {}), pendingChatRequest };
  }
  return { ...decodeLegacyLiveRequest(value, sampleSourceSelected), ...(activeDocumentContext ? { activeDocumentContext } : {}) };
}
